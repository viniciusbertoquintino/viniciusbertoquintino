...
- detecção de gestos em tempo real com landmarks MediaPipe;
- lógica de classificação por dedos levantados;
- monitor de atenção complementar (`attention_monitor.py`);
- base para evolução com testes, lint e CI.

## O que **não** é

- Sistema de controle gestual em produção
- Aplicativo mobile nativo
- Solução enterprise com múltiplas câmeras

## Estado atual

| Item | Valor |
|---|---|
| Projeto | Reconhecedor de Gestos |
| ID prefixo | RG |
| Status | Funcional (em evolução) |
| Última etapa concluída | RG.01 — código base funcional |
| Próxima etapa | RG.02 — `pyproject.toml` e `uv` |
| Superfície | Script Python + webcam |

## Roadmap

Fases em [`ROADMAP.md`](./ROADMAP.md): Base, Qualidade, Refatoração, Produção.
